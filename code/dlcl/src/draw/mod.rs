...
use embedded_graphics::{prelude::Point, pixelcolor::Rgb888, primitives::{PrimitiveStyle, Line, Primitive, Circle}};
...
///
pub fn draw_layers(layers: &[Layer]) {
    ...
    for layer in layers {
        layer.framebuf().to_pixels().iter().for_each(|p| {
